...
class Calib:
    def __init__(self):
        his_data = pd.read_csv("./HisData/experiment_data.csv")
        self.T_air_in = his_data["ACU_Return_Temp"]
        self.m_air = his_data["FCU_air_mass_flowrate"]
        self.T_water_in = his_data["chws_temperature"]
        self.m_water = his_data["chw_flow_rate_calculated"]

        self.T_water_out = his_data['chwr_temperature']
        self.T_air_out = his_data['ACU_Supply_Temp']
        self.data_length = self.T_air_in.shape[0]
    ...
    def objective(self, param):
        ...
        Tw_out_list, Ta_out_list = [], []
        for i in range(self.data_length):
            air_rho = PropsSI(
                'D', 'P', 101325, 'T', self.T_air_in[i] + 273.15, "air"
            )
            water_rho = PropsSI(
                'D', 'P', 101325, 'T', self.T_water_in[i] + 273.15, "water"
            )

            Tw_out, Ta_out, _, _, _, _ = model.forward(T_air_in=self.T_air_in[i],
                                                       m_air=self.m_air[i] * air_rho,
                                                       T_water_in=self.T_water_in[i],
                                                       m_water=self.m_water[i] * water_rho / 60 / 60)

            Tw_out_list.append(Tw_out)
            Ta_out_list.append(Ta_out)
...
if __name__ == "__main__":
    ...
    # params = [0.05,  # tubeDiameter
    #           1,  # tubeLength
    #           0.002,  # tubeThickness
    #           20,  # numberRows
    #           0.02,  # rowPitch
    #           6,  # transverseNumber
    #           0.03,  # transversePitch
    #           400  # thermalConductivity
    #           ]

    params = [0.009525,  # tubeDiameter
              1.232,  # tubeLength
              0.001,  # tubeThickness
              4,  # numberRows
              0.05,  # rowPitch
              18,  # transverseNumber
              0.04,  # transversePitch
              400  # thermalConductivity
              ]

    calib = Calib()
    calib.objective(params)
