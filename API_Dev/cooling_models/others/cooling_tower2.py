...
class CoolingTowerModel(nn.Module):
    ...
    def forward(
            self,
            water_inlet_temperature: torch.Tensor = None,
            water_outlet_temperature: torch.Tensor = None,
            air_inlet_temperature: torch.Tensor = None,
            relative_humidity: torch.Tensor = None,
            water_mass_flow_rate: torch.Tensor = None,
            fan_rated_mass_flow_rate: torch.Tensor = None,
            fan_station: bool = True,
            wind_velocity: torch.Tensor = None,
            ambient_pressure=101712.27
    ):
        air_humidity = HAPropsSI('W', 'T', air_inlet_temperature + 273.15, 'P', ambient_pressure, 'RH',
                                 relative_humidity)
        print(f"air humidity is {air_humidity}")
        if fan_station:
            summary_error = inf
            ...
            while True:
                ...
                while water_inlet_mass_flow_rate <= water_mass_flow_rate_top:
                    ...
                    while air_inlet_mass_flow_rate <= air_mass_flow_rate_top:
                        k += 1
                        print(f"i is {i}, j is {j}, k is {k},mw is {water_inlet_mass_flow_rate}, ma is {air_inlet_mass_flow_rate}")
                        print(f"ma top is {air_mass_flow_rate_top}, delta ma is {air_mass_flow_rate_difference}")
                        try:
            ...
            while True:
                ...
                while air_inlet_mass_flow_rate <= air_mass_flow_rate_top:
                    jj += 1
                    print(f"ii is {ii}, jj is {jj}, main is {air_inlet_mass_flow_rate}, mabottom is {air_mass_flow_rate_bottom}, matop is {air_mass_flow_rate_top}")
                    try:
                        merkel_number = self.get_merkel_number(
...
if __name__ == '__main__':
    ...
    water_outlet_temperature = 30
    water_inlet_temperature = 36
    air_inlet_temperature = 28
    ambient_pressure = 84100
    relative_humidity = HAPropsSI('R', 'T', air_inlet_temperature + 273.15, 'W', 0.016127, 'P', ambient_pressure)
    print(f"relative humidity 1 is {relative_humidity}")
    wet_bulb_temperature = HAPropsSI('B', 'T',  air_inlet_temperature + 273.15,
                                     'RH', relative_humidity, 'P', ambient_pressure)-273.15
    water_mass_flow_rate = 0.5 #12500
    ...
    print(f"relative humidity 2 is {relative_humidity}")