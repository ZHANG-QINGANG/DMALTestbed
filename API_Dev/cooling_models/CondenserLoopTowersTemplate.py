...
class CalculateVariableSpeedTower:
    ...
    def calculate(self, WaterMassFlowRate=1.0, Node_WaterInletNodeTemp=32, AirWetBulb=26.6, TempSetPoint=30):
        ...
        return DotMap(dict(FanPower=FanPower, Qactual=Qactual, OutletWaterTemp=OutletWaterTemp,
                           FanCyclingRatio=FanCyclingRatio, airFlowRateRatio=airFlowRateRatio))
...
if __name__ == "__main__":
    ...
    config_spec = DotMap({
        ...
        "designWaterFlowRate": 0.151,  # m3/s
        "designAirFlowRate": 95.3,  # m3/s
        "designFanPower": 29812,
    ...
    CWMassFlowRate = df["CONDENSER WATER LOOP SUPPLY OUTLET NODE:System Node Mass Flow Rate [kg/s](TimeStep)"].tolist()
    TowerInletTemp = df["CH-1F-01:Chiller Condenser Outlet Temperature [C](TimeStep)"].tolist()
    TowerOutletTemp = df["CH-1F-01:Chiller Condenser Inlet Temperature [C](TimeStep)"].tolist()
    EnvWebBulb = df["Environment:Site Outdoor Air Wetbulb Temperature [C](TimeStep)"].tolist()
    TowerFanPower = df["CT-01:Cooling Tower Fan Electricity Rate [W](TimeStep)"].tolist()

    TowerFanPowerPredict = []
    for i in range(0, len(CWMassFlowRate)):
        results = tower.calculate(WaterMassFlowRate=CWMassFlowRate[i],
                                  Node_WaterInletNodeTemp=TowerInletTemp[i],
                                  AirWetBulb=EnvWebBulb[i],
                                  TempSetPoint=32)

        TowerFanPowerPredict.append(results.FanPower)

    plt.plot(TowerFanPower, label='EnergyPlus', color='k', linestyle='-', alpha=0.8)
    plt.plot(TowerFanPowerPredict, label='Prediction', color='red', linestyle='-', alpha=0.5)

    # 添加标签和标题
    plt.xlabel('Time Step', fontsize=20)
    plt.ylabel("Cooling Tower Fan Power (W)", fontsize=20)

    plt.ylim(300, 1900)
    plt.xlim(0, len(CWMassFlowRate))
    plt.legend()
    plt.grid(True)
    plt.show()
